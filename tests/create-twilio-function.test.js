  ...
  describe('with an acceptable project name', () => {
    ...
    describe('typescript', () => {
      it('scaffolds a Twilio Function', async () => {
        ...
        const nvmrc = await stat(path.join(scratchDir, name, '.nvmrc'));
        expect(nvmrc.isFile());
        const tsconfig = await stat(path.join(scratchDir, name, 'tsconfig.json'));
        ...
        const functions = await stat(path.join(scratchDir, name, 'src', 'functions'));
        expect(functions.isDirectory());

        const assets = await stat(path.join(scratchDir, name, 'src', 'assets'));
        expect(assets.isDirectory());

        const example = await stat(path.join(scratchDir, name, 'src', 'functions', 'hello-world.ts'));
        expect(example.isFile());

        const asset = await stat(path.join(scratchDir, name, 'src', 'assets', 'index.html'));
        expect(asset.isFile());
      ...
      it('scaffolds an empty Twilio Function', async () => {
        ...
        const functionsDir = await readdir(path.join(scratchDir, name, 'src', 'functions'));
        expect(functionsDir.length).toEqual(0);

        const assetsDir = await readdir(path.join(scratchDir, name, 'src', 'assets'));
        expect(assetsDir.length).toEqual(0);
